...
from .core import (triu_edges, edge_components, _edge_trend_t, _glm_statmap, cluster_run, tfce_e_for,
                   _fl_design, build_scheme, freq_product_adjacency)
from .spatial import spatial_trend, spatial_freedman_lane

__all__ = ["triu_edges", "edge_components", "edge_adjacency", "nbs_glm", "nbs_trend",
           "nbs_freedman_lane", "nbs_step",
           "nbs_freq_trend", "nbs_freq_freedman_lane"]
...
def nbs_glm(Y, design, test_cols, iu, ju, n_nodes, *, nuisance_cols=None,
            ...
            scheme=None, ds_labels=None, restat=None, vg=None,
            blocks=None, exchange="within", units=None):
    """Per-edge GLM, thresholded and clustered on the edge graph, subnetwork-level FWER.

    stat {'t','F','W','G','rank'} ; nuisance handled by Freedman-Lane (default) or a design-based
    ...
    stat / vg : la statistique par arete et ses groupes de variance, transmis tels quels a
    _glm_statmap. Defauts inchanges ('F', aucun groupe).
    """
    statmap, effmap, Yv, Z, pZ, thr, signed = _glm_statmap(Y, design, test_cols, nuisance_cols,
                                                           stat, thresh, primary_p, vg)
    # TFCE on an EDGE / NBS graph : no spatial lattice dimension applies, so treat it as a 2D-analogue
    # (E=2/3) - the neighbourhood is the incident-edge clique, denser than a 1D chain (dim=2 default).
    tfce_e = tfce_e_for(None, dim=2) if (isinstance(thr, str) and thr == "TFCE") else None
...
def nbs_trend(effects, iu, ju, n_nodes, session_idx=None, t_thresh=2.5, primary_p=None,
              ...
              cluster_stat="size", stat=None, vg=None):
    ...
    stat : None (defaut) laisse `method` decider comme avant ; une valeur explicite l'emporte.
    ...
    res = nbs_glm(effects, design, [1], iu, ju, n_nodes, nuisance_cols=[0], stat=st, vg=vg,
...
def nbs_freedman_lane(Y, iu, ju, n_nodes, *, effect, nuisance,
    ...
    stat : 'auto' (defaut) est resolu par _fl_design en 't' ou 'F' ; 'W' et 'G' le traversent
           intacts et designent la statistique elementaire de _glm_statmap.
    vg   : groupes de variance par observation, transmis a la carte observee ET au recalcul de
           chaque relabelisation Draper-Stoneman.
    """
    ...
    res = nbs_glm(Y, design, test_cols, iu, ju, n_nodes, nuisance_cols=nuis_cols,
                  stat=stat, vg=vg, thresh=thresh, primary_p=primary_p, n_perm=n_perm,
...
def nbs_step(Y, onset, time, iu, ju, n_nodes, *, adjust_time=True, t_thresh=None,
    ...
    stat : None (defaut) -> 't', la colonne de marche etant unique ; 'W' / 'G' la remplacent.
    vg   : groupes de variance transmis a _glm_statmap.
    """
    ...
    res = nbs_glm(Y, design, [tcol], iu, ju, n_nodes, nuisance_cols=nuis,
                  stat=("t" if stat is None else stat), vg=vg,
...
def nbs_freq_trend(effects, iu, ju, n_nodes, n_freqs, *, t_thresh=2.5, primary_p=None, n_perm=1000,
                   ...
                   adjacency=None, stat=None, vg=None):
    ...
    stat / vg : transmis a spatial_trend (None = `method` decide, comme avant)."""
...
def nbs_freq_freedman_lane(Y, iu, ju, n_nodes, n_freqs, *, effect, nuisance,
                           ...
                           max_consecutive=None, adjacency=None, vg=None):
    ...
    stat / vg : transmis a spatial_freedman_lane (defauts inchanges)."""
    ADJ = adjacency if adjacency is not None else freq_product_adjacency(
        edge_adjacency(iu, ju, n_nodes), n_freqs)
    res = spatial_freedman_lane(Y, ADJ, effect=effect, nuisance=nuisance,
                                ...
                                vg=vg, thresh=thresh, primary_p=primary_p, n_perm=n_perm,
                                alpha=alpha, tail=tail, seed=seed, cluster_stat=cluster_stat,
